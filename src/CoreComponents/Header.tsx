...
const getNavigationNodesStatic = (siteUrl: string): INavigationNode[] => [
  {
    Id: 1,
    Title: "Employee Hub",
    TitleAr: "بوابة الموظف",
    Url: siteUrl + "/SitePages/CustomHome.aspx",
    IsExternal: false,
    Children: [],
  },
  {
    ...
    Children: [
      {
        ...
        Title: "News",
        TitleAr: "الأخبار",
        Url: siteUrl + "/_layouts/15/news.aspx",
        IsExternal: false,
        Children: [],
      },
      {
        ...
        Title: "Announcements",
        TitleAr: "الإعلانات",
        Url: siteUrl + "/SitePages/Announcements.aspx",
        IsExternal: false,
        Children: [],
      },
      {
        Id: 23,
        Title: "Internal Jobs",
        TitleAr: "الوظائف الداخلية",
        Url: siteUrl + "/SitePages/Internal-jobs.aspx",
        IsExternal: false,
        Children: [],
      },
      {
  ...
  },
  {
    ...
    Children: [
      {
        Id: 31,
        Title: "Matari Program",
        TitleAr: "برنامج مطاري",
        Url: "#",
        IsExternal: false,
        Children: [],
      },
  ...
  },
  {
    ...
    Title: "About",
    TitleAr: "عن الهيئة",
    Url: siteUrl + "/SitePages/About-Us.aspx",
    IsExternal: false,
    Children: [
      {
        Id: 41,
        Title: "SAA Strategy",
        TitleAr: "استراتيجية الهيئة",
        Url: "#",
        IsExternal: false,
        Children: [],
      },
      {
        Id: 42,
        Title: "SAA Organizational Structure",
        TitleAr: "الهيكل التنظيمي للهيئة",
        Url: "#",
        IsExternal: false,
        Children: [],
      },
...
class HeaderPageComponent extends React.Component<IHeaderProps, IHeaderState> {
  constructor(props: IHeaderProps) {
    super(props);
    const autoLang = window.location.href.toLowerCase().includes("/ar/")
      ? "ar"
      : "en";
    this.state = {
      langOpen: false,
  ...
  private loadNavigation = async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        ...
        this.setState({
          navigationNodes: getNavigationNodesStatic(siteUrl) || nodes,
          loading: false,
        });
      }
    } catch (error) {
  ...
  private handleSearch = (value: string) => {
    ...
    }/_layouts/15/search.aspx?q=${encodeURIComponent(value)}`;
  ...
  private openWaffle = (): void => {
    ...
    // If the Microsoft 365 shell provides the waffle launcher, use it
    if (
      anyWindow._spLaunchWaffle &&
      typeof anyWindow._spLaunchWaffle === "function"
    ) {
      anyWindow._spLaunchWaffle(); // This opens the apps panel like in your screenshot
      return;
    }

    // Fallback: open the apps page in a new tab
    const appsUrl = "https://www.office.com/apps";
  ...
  //   if (lng === "ar") {
  //     // Redirect to Arabic version: insert "/ar/" before the page name if not present
  //     if (!currentUrl.toLowerCase().includes("/ar/")) {
  //       newUrl = currentUrl.replace(/\/SitePages\//i, "/SitePages/ar/");
  //     }
  //   } else {
  //     // Redirect to English version: remove "/ar/" from the URL if present
  //     newUrl = currentUrl.replace("/SitePages/ar/", "/SitePages/");
  //   }
  ...
  // Check if a given page URL exists (returns false on 404 or any error)
  private async pageExists(absoluteUrl: string): Promise<boolean> {
    try {
      const response = await fetch(absoluteUrl, {
        method: "HEAD",
        credentials: "same-origin",
      });
      return response.ok; // false for 404/500 etc.
    } catch (error) {
  ...
  private changeLang = async (lng: "en" | "ar"): Promise<void> => {
    const currentUrl = window.location.href;
    let targetUrl = currentUrl;

    if (lng === "ar") {
      // EN → AR : insert /ar/ if not there
      if (!currentUrl.toLowerCase().includes("/sitepages/ar/")) {
        targetUrl = currentUrl.replace(/\/SitePages\//i, "/SitePages/ar/");
      }
    } else {
      // AR → EN : remove /ar/
      targetUrl = currentUrl.replace(/\/SitePages\/ar\//i, "/SitePages/");
    }
    ...
    const exists = await this.pageExists(targetUrl);

    if (exists) {
      // Page exists → safe to redirect
      this.setState({ selectedLang: lng, langOpen: false });
      this.applyLanguageDirection(lng);
      window.location.href = targetUrl;
    } else {
      // Page does NOT exist → handle gracefully

      // OPTION A: show a message and stay on current page
      alert(
        lng === "ar"
          ? "Arabic version of this page is not available yet."
          : "English version of this page is not available yet."
      );
    }
  };
    ...
    return (
      <div className="navigationContainer">
          ...
          <div className="top-Bar">
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  ).src = require("../theme/images/logo.svg");
                }}
              />
            </a>
            <img
              src={require("../theme/images/triangles.svg")}
              className="traingleBg"
            />
                ...
                <div className="user-box">
                  <img
                    ...
                    onError={(e) => {
                      (
                        e.target as HTMLImageElement
                      ).src = require("../theme/images/default-user.jpg");
                    }}
                    ...
                    <div className="curr-user">
                      <img
                        ...
                        onError={(e) => {
                          (
                            e.target as HTMLImageElement
                          ).src = require("../theme/images/default-user.png");
                        }}
                      />
                      <div>
            ...
            <div className="lang-switcher">
              <div className="selected-lang" onClick={this.toggleLangDropdown}>
                {this.state.selectedLang.toUpperCase()}{" "}
                ...
                <ul className="lang-dropdown">
                  {["en", "ar"]
                    ...
                    .map((lang) => (
                      <li
                        key={lang}
                        onClick={() => this.changeLang(lang as "en" | "ar")}
                      >
                        {lang.toUpperCase()}
                      </li>
                    ))}
        ...
        <div className={this.state.menuOpen ? "mobileMenu open" : "mobileMenu"}>
          <ul>{this.renderNavigationItems(navigationNodes)}</ul>
        </div>
      </div>
    );
  }
